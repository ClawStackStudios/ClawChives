...
export class MigrationRunner {
  ...
  /**
   * Initialize migrations log table and handle legacy migration detection
   */
  private initSchemaTable() {
    ...
    // If the database is a pre-existing ClawChives vault (users table exists),
    // but the schema_migrations table is empty, seed version 1 as completed.
    const migrationCount = this.db.prepare('SELECT COUNT(*) as count FROM schema_migrations').get() as { count: number };
    if (migrationCount.count === 0) {
      const usersTableExists = this.db.prepare(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='users'"
      ).get();

      if (usersTableExists) {
        console.log('[DB Migration] 🦞 Legacy database detected. Seeding baseline migration v1.');
        this.db.prepare(`
          INSERT INTO schema_migrations (version, name, applied_at)
          VALUES (1, '0001_initial', datetime('now'))
        `).run();
      }
    }
  }
