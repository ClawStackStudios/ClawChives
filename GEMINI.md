...
**Security Middleware:**
* `requireAuth`: Validates `api-` token and attaches `req.agentPermissions`.
...
- [x] **Docker Compose:** Containerized orchestration (`dev` and `api`).
...
- [x] **API Security:** All routes protected by `requirePermission` middleware. Identites and keys downloadable locally.
- [x] **Liquid Metal Toggle:** Circular reveal animations.
